...
const Acc = () => {
  ...
  // API에서 ACC 카테고리 상품 데이터 가져오기
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://localhost:5000/api/products?category=ACC');
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#fff' }}>
      ...
      >
        <h1 style={{ fontSize: isMobile ? '1.5rem' : '2rem', marginBottom: '2rem' }}>ACC</h1>
        <div
          style={{
            display: 'grid',
