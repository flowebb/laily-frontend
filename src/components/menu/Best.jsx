...
const Best = () => {
  ...
  // API에서 BEST 상품 데이터 가져오기 (예: status에 BEST가 포함된 상품)
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (data.success) {
          // BEST 상품 필터링 (status에 'BEST'가 포함된 상품)
          const bestProducts = data.products.filter(product => 
            product.status && product.status.includes('BEST')
          );
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#fff' }}>
      ...
      >
        <h1 style={{ fontSize: isMobile ? '1.5rem' : '2rem', marginBottom: '2rem', padding: isMobile ? '0 1rem' : '0' }}>BEST</h1>
        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#666' }}>
            <p>등록된 BEST 상품이 없습니다.</p>
          </div>
        ) : (
          <div
            style={{
